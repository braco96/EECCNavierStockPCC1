...
def transformar_poligono_a_limites(square_coordinates):
    """
    Transforma un polígono definido por una lista de coordenadas (longitud, latitud) 
    a un formato de límites: 'S, O, N, E'.
    
    Parámetros:
    polygon_coords (list of tuples): Lista de coordenadas del polígono en formato [(long, lat), ...]

    Retorna:
    str: Coordenadas en formato 'S, O, N, E'
    """
    longitude = [square_coordinates['nw']['longitude'],square_coordinates['ne']['longitude'],square_coordinates['se']['longitude'],square_coordinates['sw']['longitude']]
    latitude =  [square_coordinates['nw']['latitude'],square_coordinates['ne']['latitude'],square_coordinates['se']['latitude'],square_coordinates['sw']['latitude']]
    ...
    # Formatear y devolver el resultado como string
    return f'{lat_sur}, {long_oeste}, {lat_norte}, {long_este}'

# Usar la función
resultado = transformar_poligono_a_limites(get_satelite_polygon_coordinates(madrid_muestra))
print(resultado)
...
import os
import subprocess
def modificar_settings_y_ejecutar_acolite(madrid_muestra:Muestras, elevation:int,resultado):
    # Directorio actual
    current_dir = os.getcwd()
    ...
    limit_str = '40.43918148084956,-4.258400981846755,40.539181481065874,-4.058400981846755'
    ...
    settings_content = settings_content.replace('${INPUT-FILE}',  '/input/S2B_MSIL1C_20220131T111209_N0400_R137_T30TVK_20220131T115433.SAFE' )  # Ruta completa a la carpeta .SAFE
    settings_content = settings_content.replace('${ELEVATION}', str(elevation))  # Establecer la altura a 600
    settings_content = settings_content.replace('${LIMIT}', transformar_poligono_a_limites(get_satelite_polygon_coordinates(madrid_muestra)))  # Reemplazar la clave limit con las coordenadas
...
modificar_settings_y_ejecutar_acolite(madrid_muestra,"60",resultado)
