...
def contiene(grande, peque):
    # Crear los objetos Polygon de Shapely
    print('polygono grande ----------->',grande)
    print('polygono grande ----------->',peque)
    outer_gran = Polygon(grande[0])
    inner_peq = Polygon(peque[0])
    
    # Verificar si el polígono interior está completamente contenido dentro del polígono exterior
...
def coger(response_json,muestra):
    ...
    if not entries:
        ...
        return []
    for i in entries:
        if(muestra.cantidad>len(muestra.informaciones)):
            fileName = i['Name']
            ...
            if(contiene(i['GeoFootprint']['coordinates'],muestra.polygono)):
...
def get_satelite_muestras(muestra):
        ...
        filter_query = (f"$filter=Collection/Name eq '{modo[2]}' and "
                        f"contains(Name,'{modo[3]}') and Attributes/OData.CSC.DoubleAttribute/any(att:att/Name eq 'cloudCover' and att/OData.CSC.DoubleAttribute/Value le 1.00) and "
...
def crear_carpetas_y_archivos():
    ...
    archivos_contenido = {
'settings.txt.tpl': """EARTHDATA_u=bravo1996
...
blackfill_wave=1600
output_bt=False
l2w_parameters=cdom_*,tur_nechad2009_*,avw_vis_*,avw_visnir_*,chl_toming_*,chl_se2waq_*,sd_*,tss_*
, l2r_export_geotiff_rgb=True,l1r_delete_netcdf=True,l2r_delete_netcdf=True,l2w_delete_netcdf=True,rgb_rhow=True,merge_tiles=True,map_l2w=True,s2_target_res=10
...
l2w_mask=True
l2w_mask_wave=1600
l2w_mask_threshold=0.0000000215
l2w_mask_water_parameters=True
l2w_mask_negative_rhow=True
l2w_mask_negative_wave_range=400,900
l2w_mask_cirrus=True
l2w_mask_cirrus_threshold=0.00005
l2w_mask_cirrus_wave=1373
l2w_mask_high_toa=True
l2w_mask_high_toa_threshold=000.3
l2w_mask_high_toa_wave_range=400,2500
l2w_mask_mixed=True
l2w_data_in_memory=False
nechad_range=600,900
...
descargar(madrid_muestra)  
ejecutar_acolite(madrid_muestra)